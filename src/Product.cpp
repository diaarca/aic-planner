                 ...
                 const std::vector<Mineral>& mineral_limits)
{
    ...
    try
    {
        ...
        in.read_header(io::ignore_extra_column, "product", "value", "time",
                       "originium", "amethyst", "ferrium", "depot", "width",
                       "height", "zipline", "mining_rig", "protocol_stash",
                       "refining_unit", "shredding_unit", "moulding_unit",
                       "seed_picking_unit", "gearing_unit", "fitting_unit",
                       "planting_unit", "filling_unit", "packaging_unit",
                       "grinding_unit");

        std::string name;
        double value, time, originium, amethyst, ferrium, depot, width, height;
        double zipline, mining_rig, protocol_stash, refining_unit,
            shredding_unit, moulding_unit, seed_picking_unit, gearing_unit,
            fitting_unit, planting_unit, filling_unit, packaging_unit,
            grinding_unit;

        while (in.read_row(name, value, time, originium, amethyst, ferrium,
                           depot, width, height, zipline, mining_rig,
                           protocol_stash, refining_unit, shredding_unit,
                           moulding_unit, seed_picking_unit, gearing_unit,
                           fitting_unit, planting_unit, filling_unit,
                           packaging_unit, grinding_unit))
        {
            Product prod;
            prod.name = name;
            prod.value = value;
    ...
    {
        std::cerr << "Error reading products from " << filename << ": "
                  << e.what() << std::endl;
    }
    return products;
}
